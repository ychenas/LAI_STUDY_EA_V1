...
par(mar = c(4, 4, 0, 0))
   plot(x=x_pos , y=y_pos, xlab="SoilWater_bef", ylab="WindSpeed", cex.lab=1.5, cex.axis=1.5,
        col="black", bg=table.comb$col, xlim=c(0.25,0.4), ylim=c(2,12), cex=eff_cex, pch=21,lwd=1.5)
   grid()
...
par(mar = c(0, 4, 3, 0))
   ...
   plot(y=table.comb$eff.size.for, x=table.comb$bef.for.lai.aff,
         ...
         col="white",ylim=c(-1.5,1.5),xlim=c(0.25,0.4),
   ...
   sm.ln <- smooth.spline(y=table.comb$eff.size.for, x=table.comb$bef.for.asw.aff, df=10)
...
par(mar = c(4, 0, 0, 0))
   ...
   plot(x=table.comb$eff.size.for, y=table.comb$eve.for.mws.aff,ylim=c(2,12), 
   ...
   sm.ln <- smooth.spline(y=table.comb$eff.size.for,x=table.comb$eve.for.mws.aff, df=10)
   ...
   table.comb$date.day <- substr(table.comb$date.time,start=7,stop=8)
   #subset datataset
   table.comb <- subset(table.comb, ((table.comb$eve.for.pix.aff)>2000 & as.integer(as.character(table.comb$date.time)) <= 20180101  )  )
   table.comb <- subset(table.comb, (abs(table.comb$qc1.score)<=0.2 &  abs(table.comb$qc2.score)>=0.2 ))
