...
# Revised Date: 2020-06-29 add quality check flags (comment by CHWu) 
...
# 4. load Monthly SPEI maps over the East Asia
       ...
       wrk.date.spei <- paste(substr(lai.fnames[it+1], start=11, stop=14),"_",substr(lai.fnames[it+1], start=15, stop=16), sep="") 
       spei.data <- fun_read_nc(arg1=paste(wrk.dir,"/SPEI_DATA/",spei.fname,"_", wrk.date.spei,"_EA.nc",sep=""), var_st=1)
 
       wrk.date.spei.pri <- paste(substr(lai.fnames[it-1], start=11, stop=14),"_",substr(lai.fnames[it-1], start=15, stop=16), sep="") 
       spei.data.pri <- fun_read_nc(arg1=paste(wrk.dir,"/SPEI_DATA/",spei.fname,"_", wrk.date.spei.pri,"_EA.nc",sep=""), var_st=1)
...
#}
  ...
  #=================================
   ...
   eve.for.spei.aff.pri  <- mean( spei.data.pri$SPEI*eve.tc.aff.mask*lc.for.mask,na.rm=T) 
     print( paste("Forests Mean SPEI in affected area:", eve.for.spei.aff.pri) ) 
...
if (ld_go) {
  ...
  if ( is.na(eff.size.for) != TRUE ) {
     table.tmp <- data.frame( date.time = wrk.date,
                              ...
                              eve.for.spei.aff.pri = eve.for.spei.aff.pri,
